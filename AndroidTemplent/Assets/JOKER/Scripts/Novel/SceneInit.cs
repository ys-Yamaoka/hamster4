...
public class SceneInit : MonoBehaviour {
	...
	public float messageSpeed = 0.05f; //メッセージ速度

	private string currentMessage = ""; //現在表示中の文字列
    private string currentMessage2 = ""; //現在表示中の文字列
    ...
    public void coroutineShowMessage2(string message2){
        StatusManager.isMessageShowing = true;
        StartCoroutine("showMessage2", message2);
    }
    ...
    //メッセージ表示制御用のコルーチン
    private IEnumerator showMessage2(string message)
    {
        ...
        for (int i = 0; i < message.Length; i++)
        {
            ...
            this.gameView.messageAreaPlayer.text = this.currentMessage;
            yield return new WaitForSeconds(this.messageSpeed);
        }
